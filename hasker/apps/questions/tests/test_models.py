...
@pytest.mark.django_db
def test_get_trending_questions(person, trending_questions):
...
@pytest.mark.django_db
def test_find_question_by_title(person, question):
...
def test_mark_answer_as_correct_ok(person, another_person, question):
    ...
    upd_question = Question.objects.get()
    assert upd_question.get_correct_answer() == answer
